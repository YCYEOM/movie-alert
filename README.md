...
빠른 감시는 비용을 아끼려고 미오픈 날짜 중 앞 `fast_dates`일만 봅니다. 다만 **거기서 오픈이
감지되면 남은 미오픈 날짜까지 즉시 마저 훑어 한 통으로 보냅니다.** 예매는 보통 여러 날이
한꺼번에 열리는데, 2일씩 쪼개 여러 통으로 오면 전체 그림을 보기 어렵기 때문입니다.
요청이 늘어나는 것은 실제 오픈이 일어난 순간뿐입니다. 예매가 열리면 그 날짜에 회차가 통째로 생기므로 그대로 잡힙니다.
