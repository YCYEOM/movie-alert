...
**tick 마다 요청을 정확히 1건씩만 씁니다.** 예전엔 30분마다 126요청을 50초에 몰아 쏘고
4초마다 12요청을 더 썼는데(약 3 req/s), CGV 앞단 Cloudflare 레이트리밋(`error code: 1015`)에
걸렸습니다. 같은 총량을 tick 단위로 흩뿌리면 폭주 구간이 사라집니다.

tick 은 두 가지 일을 번갈아 합니다.

### 트립와이어 — 요청 1건으로 오픈을 감지

```
GET /api/v1/booking/searchAtktTopPostrList?coCd=A420&div=CUST_EXPO_MOVTYP_CD&attrCd=04
    -> 222B. "지금 IMAX 로 예매 가능한 영화" (siteNo 불필요, CGV 전체)

GET /api/v1/booking/searchSiteScnscYmdListBySite?coCd=A420&siteNo=0013
    -> 767B. "그 극장에 상영 일정이 잡힌 날짜" (scnYmd 불필요)
```

`attrCd` 는 `04`=IMAX, `03`=4DX, `08`=SCREENX, `06`=DOLBY ATMOS 입니다.
**아바타가 IMAX 목록에 뜨는 순간이 곧 IMAX 예매 오픈입니다.** 이때만 해당 극장의
전 날짜를 몰아서 훑습니다 — 평소에 요청을 아끼는 이유가 이 순간에 쓰기 위해서입니다.

날짜 목록은 예매가 열리기 **전에** 먼저 뜹니다. 덕분에 사전 신호로 쓸 수 있고,
`days_ahead` 같은 임의의 창을 둘 필요도 없습니다.

### 스윕 커서 — 이미 열린 날짜에 붙는 회차

```
GET /api/v1/booking/searchMovScnInfo?coCd=A420&siteNo=0013&scnYmd=20260902&rtctlScopCd=01
```

`(극장, 날짜)` 한 쌍씩 돌아가며 훑고, 직전 스냅샷에 없던 회차만 알립니다.
**이 경로가 실제로 가장 자주 쓰입니다.** 8/28→8/31 실측에서 용산의 신규 회차 43건이
전부 *이미 예매가 열려 있던* 09/02·09/03·09/05 에 붙었습니다. 새 영화 예매 오픈은
대개 새 날짜가 아니라 기존 날짜에 붙으므로, 날짜 목록만 봐서는 못 잡습니다.

날짜 목록이 알려준 실제 날짜만 돌기 때문에 6극장 × 평균 17일 ≈ 99쌍이고,
tick 0.75초 기준 한 바퀴가 약 1분 50초입니다. 사라진 회차는 로그도 알림도 남기지 않습니다.

### 설정

| 키 | 기본 | 뜻 |
|---|---|---|
| `tick_seconds` | `0.75` | tick 간격. 그대로 요청 속도(≈1.3 req/s)가 됩니다 |
| `sweep_per_tripwire` | `2` | 스윕 2틱마다 트립와이어 1틱 |
| `days_ahead` | `60` | 날짜 목록에서 이 범위까지만 봅니다 |
| `notice_seconds` | `300` | 예매 오픈 공고 확인 주기 |

**`tick_seconds` 를 줄이면 그만큼 빨라지고 그만큼 요청이 늘어납니다.** 3 req/s 에서
1015 에 걸린 전력이 있고, **1 req/s 에서도 KST 14~17시 피크에 걸렸습니다**(9/1~9/2,
쿨다운 117회). 429 한 번은 30~60초 완전 실명이라 감지가 오히려 느려지므로,
줄인 뒤엔 며칠간 `journalctl | grep 429` 로 확인하세요. 걸리기 시작하면 되돌리는
편이 빠릅니다.
...
특별관 예매는 두 단계로 일어나고, 이 둘은 다른 사건이라 채널을 나눌 수 있습니다.

| | 무엇 | 주기 | 채널 환경변수 |
|---|---|---|---|
| **예매 오픈 공고** | "O월 O일 O시 오픈" 사전 안내 | 5분 | `NOTICE_WEBHOOK_URL` |
| **실제 회차 오픈** | 좌석이 실제로 열림 | 4초 | `DISCORD_WEBHOOK_URL` |

공고는 이벤트 게시판(`/api/v1/content/event/...`)에서 제목에 `notices.keywords`가
들어간 새 글을 찾습니다. 기본 키워드는 `예매·아이맥스·IMAX·4DX·SCREENX·돌비·특별관`이고,
시끄러우면 `config.json`에서 줄이면 됩니다.

주의 — 공고가 이벤트 게시판이 아닌 곳(영화 상세, SNS)에만 올라오면 못 잡습니다.
회차 오픈(아래)은 그와 무관하게 항상 잡힙니다.
...
- `tick_seconds`를 무리하게 줄이지 마세요. tick 당 요청 1건이라 그대로 초당 요청 수가 됩니다.
  CGV 는 3 req/s 에서 `error code: 1015` 로 막았습니다.
...
`tick_seconds: 0.75` 기준입니다 (2026-09-03, 6극장 99쌍).

| 무엇 | 주기 | 요청 |
|---|---|---|
| 특별관 예매 오픈 (**아바타 IMAX**) | **약 20초** + 즉시 펼침 | 222~750B × 1건 |
| 새 날짜 열림 | 약 20초 + 즉시 펼침 | 767B × 1건 |
| 이미 열린 날짜에 회차 추가 | 약 1분 50초 | 8KB × 1건 |
| 예매 오픈 공고 | 5분 | 1건 |

트립와이어 9종(특별관 3 + 극장 6)을 3틱에 한 번씩 도니 한 바퀴가 20초,
스윕 커서 99쌍을 나머지 틱으로 도니 한 바퀴가 약 1분 50초입니다.
합쳐서 **약 1.3 req/s, 폭주 구간 없음.**

**새 날짜는 그 자체가 좌석 오픈 신호입니다.** 9/3 실측에서 용산 9/14 가 목록에 뜬
1초 뒤 IMAX 회차 5건이 잡혔습니다. 3일간 20건 발생했고, 새 영화 오픈(2건)보다
훨씬 잦습니다. 그래서 트립와이어 두 종류는 같은 빈도로 돕니다.

옛 구조는 3 req/s 에 126요청 폭주가 얹혔고 21일 밖은 아예 못 봤습니다.
지금은 요청이 1/3이면서 CGV 가 일정을 잡아둔 날짜 전부를 봅니다.

### 레이트리밋에 대해 알아낸 것

- CGV 오리진에 Cloudflare 레이트리밋 규칙이 있고, 걸리면 `429` 와 `error code: 1015` 를 줍니다.
- **경로별로 걸립니다.** 걸린 상태에서도 `searchRegnList` 는 200, `searchMovScnInfo` 만 429 였습니다.
- **IP별로 걸립니다.** Worker 송신 IP가 막힌 순간에도 집 IP는 같은 경로에 200 이었습니다.
- 3 req/s 를 20분간(약 3,600요청) 유지하자 걸렸습니다. 8/28의 "3 req/s 안전" 부하 테스트는
  237요청·4분짜리라 더 긴 창을 보지 못했습니다.
- **1 req/s 도 안전지대가 아닙니다.** 9/1~9/2 에 쿨다운 117회가 걸렸는데, 전부
  KST 14~17시에 몰렸습니다. 같은 속도라도 CGV 쪽 부하가 높은 시간대엔 더 쉽게 걸립니다.
