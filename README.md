...
날짜 목록이 알려준 실제 날짜만 돌기 때문에 6극장 × 평균 14일 ≈ 83쌍이고,
tick 1초 기준 한 바퀴가 약 2분입니다. 사라진 회차는 로그도 알림도 남기지 않습니다.

### 설정

| 키 | 기본 | 뜻 |
|---|---|---|
| `tick_seconds` | `1.0` | tick 간격. 그대로 요청 속도(≈1 req/s)가 됩니다 |
...
1015 에 걸린 전력이 있으니 `1.0` 으로 시작해 며칠 무사한 걸 보고 `0.7`, `0.5` 로
조이는 편이 안전합니다. `0.5` 면 2 req/s 로 걸렸던 값에 가까워집니다.
...
`tick_seconds: 1.0` 기준 실측입니다 (2026-08-31, 6극장 83쌍).

| 무엇 | 주기 | 요청 |
|---|---|---|
| 특별관 예매 오픈 (**아바타 IMAX**) | **약 27초** + 즉시 펼침 | 222~750B × 1건 |
| 새 날짜 열림 | 약 27초 | 767B × 1건 |
| 이미 열린 날짜에 회차 추가 | 약 2분 | 8KB × 1건 |
| 예매 오픈 공고 | 5분 | 1건 |

트립와이어 9종(특별관 3 + 극장 6)을 3틱에 한 번씩 도니 한 바퀴가 27초,
스윕 커서 83쌍을 나머지 틱으로 도니 한 바퀴가 약 2분입니다.
합쳐서 **약 1.0 req/s, 폭주 구간 없음.**
